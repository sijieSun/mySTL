...
namespace mySTL
{
    ...
    /*
    all_of
    检查区间内元素是否满足一元操作符unary_pred为true的情况，满足则返回true
     */
    ...
    /* 
    search
    在[first1, last1) 中查找[first2, last2)的首次出现点，失败返回last1
     */
    ...
    search_n(ForwardIter first, ForwardIter last,
            Size n, const T& value, Compared cmp)
    {
        if (n <= 0)
        {
            return first;
    ...
    /* 
    find_end
    在 [first1, last1) 中查找 [first2, last2) 最后一次出现的地方，若不存在返回last1
     */
    ...
    find_end_dispatch(ForwardIter1 first1, ForwardIter1 last1,
                    ForwardIter2 first2, ForwardIter2 last2,
                    forward_iterator_tag, forward_iterator_tag, Compared cmp)
    {
        if (first2 == last2)
        {
            return last1;
    ...
    find_end_dispatch(BidirectionalIter1 first1, BidirectionalIter1 last1,
                    BidirectionalIter2 first2, BidirectionalIter2 last2,
                    bidirectional_iterator_tag, bidirectional_iterator_tag, Compared cmp)
    {
        typedef reverse_iterator<BidirectionalIter1> reviter1;
        typedef reverse_iterator<BidirectionalIter2> reviter2;
        reviter1 rlast1(first1);
            ...
            ForwardIter2 first2, ForwardIter2 last2, Compared cmp)
    {
        typedef typename iterator_traits<ForwardIter1>::iterator_category Category1;
        typedef typename iterator_traits<ForwardIter2>::iterator_category Category2;
        return find_end_dispatch(first1, last1, first2, last2, Category1(), Category2(), cmp);
    }


    /* 
    find_first_of
    在 [first1, last1) 中 查找 [first2, last2) 中的元素，返回第一个出现的元素的迭代器
     */
    ...
    /* 
    for_each
    使用函数对象 f 对 区间的每个元素执行operator()操作，不改变原来的值
    f（）可返回一个值，但通常被忽略
     */
    ...
    /* 
    adjacent_find
    找出第一对匹配的相邻元素，缺省适用 operator== 
    找到返回第一个元素迭代器，否则返回last
     */
    ...
    ForwardIter adjacent_find(ForwardIter first, ForwardIter last, Compared cmp)
    {
        ...
        while (++next != last)
        {
            if (cmp(*first, *next))  return first;
            first = next;
        }
        return last;
    }
    ...
    /* 
    lower_bound
    在 [first, last) 中查找第一个不小于value的元素
    成功则返回其迭代器，否则返回last
     */
    ...
    RandomIter
    lbound_dispatch(RandomIter first, RandomIter last,
                    const T& value, random_access_iterator_tag, Compared cmp)
    {
        auto len = last - first;
        auto half = len;
        RandomIter middle;
    ...
    template <class ForwardIter, class T, class Compared>
    ForwardIter
    lower_bound(ForwardIter first, ForwardIter last, const T& value, Compared cmp)
    {
        return lbound_dispatch(first, last, value, iterator_category(first), cmp);
    }
    ...
    template <class ForwardIter, class T, class Compared>
    ForwardIter
    upper_bound(ForwardIter first, ForwardIter last, const T& value, Compared cmp)
    {
        return ubound_dispatch(first, last, value, iterator_category(first), cmp);
    }

    /* 
    binary_search
    如果在[first, last)内有等同于 value 的元素，返回 true，否则返回 false
     */
    ...
    // 重载版本使用函数对象 cmp 代替比较操作
    template <class ForwardIter, class T, class Compared>
    bool binary_search(ForwardIter first, ForwardIter last, const T& value, Compared cmp)
    {
        auto i = lower_bound(first, last, value);
                    ...
                    const T& value, forward_iterator_tag)
    {
        ...
        while (len > 0)
        {
            ...
            {
                left = lower_bound(first, last, value); //  middle 到 left 全都equal
                    ...
                    const T& value, forward_iterator_tag, Compared cmp)
    {
        ...
        while (len > 0)
        {
            ...
            else if (cmp(value, *middle))
            {
                len = half;
            }
            else
            {
                left = lower_bound(first, last, value, cmp);
                mySTL::advance(first, len);
                right = upper_bound(++middle, first, value, cmp);
                return mySTL::pair<ForwardIter, ForwardIter>(left, right);
            }
        }
        return mySTL::pair<ForwardIter, ForwardIter>(last, last);
                    ...
                    const T& value, random_access_iterator_tag, Compared cmp)
    {
        ...
        while (len > 0)
        {
            ...
            {
                left = lower_bound(first, middle, value, cmp);
                right = upper_bound(++middle, first + len, value, cmp);
                return mySTL::pair<RandomIter, RandomIter>(left, right);
            }
        }
        return mySTL::pair<RandomIter, RandomIter>(last, last);
    ...
    template <class ForwardIter, class T, class Compared>
    mySTL::pair<ForwardIter, ForwardIter>
    equal_range(ForwardIter first, ForwardIter last, const T& value, Compared cmp)
    {
        return erange_dispatch(first, last, value, iterator_category(first), cmp);
    }

    /* 
    generate
    将函数对象gen的结果对区间每个元素赋值
     */
    ...
    bool includes(InputIter1 first1, InputIter1 last1,
                InputIter2 first2, InputIter2 last2, Compared cmp)
    {
        while (first1 != last1 && first2 != last2)
        {
    ...
    bool is_heap(RandomIter first, RandomIter last, Compared cmp)
    {
        auto n = mySTL::distance(first, last);
        auto parent = 0;
        for (auto child = 1; child < n; ++child)
    ...
    // 重载版本使用函数对象 cmp 代替比较操作
    template <class ForwardIter, class Compared>
    bool is_sorted(ForwardIter first, ForwardIter last, Compared cmp)
    {
        if (first == last)
            return true;
        auto next = first;
    ...
    const T& median(const T& left, const T& mid, const T& right, Compared cmp)
    {
        if (cmp(left, mid))
            if (cmp(mid, right))
                return mid;
    ...
    // 重载版本使用函数对象 cmp 代替比较操作
    template <class ForwardIter, class Compared>
    ForwardIter min_elememt(ForwardIter first, ForwardIter last, Compared cmp)
    {
        if (first == last)
            return first;
        auto result = first;
    ...
    {
        first = find(first, last, value);  // 利用 find 找出第一个匹配的地方
        auto next = first;
    ...
    {
        first = find_if(first, last, unary_pred);  // 利用 find_if 找出第一个匹配的地方
        auto next = first;
        return first == last ? first : remove_copy_if(++next, last, first, unary_pred);
    }
    ...
    {
        reverse_dispatch(first, middle, bidirectional_iterator_tag());
        reverse_dispatch(middle, last, bidirectional_iterator_tag());
    ...
    如果没有下一个排序组合，返回 false，否则返回 true
     */
    ...
    /* 
    merge
    将两个经过排序的集合 S1 和 S2 合并起来置于另一段空间
    返回一个迭代器指向最后一个元素的下一位置
     */
                            ...
                            Compared cmp)
    {
        ...
        mySTL::merge_without_buffer(first, first_cut, new_middle, len11, len22, cmp);
        mySTL::merge_without_buffer(new_middle, second_cut, last, len1 - len11, len2 - len22, cmp);
    }
    ...
    merge_backward(BidirectionalIter1 first1, BidirectionalIter1 last1,
                BidirectionalIter2 first2, BidirectionalIter2 last2,
                BidirectionalIter1 result, Compared cmp)
    {
        if (first1 == last1)
            return mySTL::copy_backward(first2, last2, result);
        if (first2 == last2)
    ...
    void merge_adaptive(BidirectionalIter first, BidirectionalIter middle,
                        BidirectionalIter last, Distance len1, Distance len2,
                        Pointer buffer, Distance buffer_size, Compared cmp)
    {
        // 区间长度足够放进缓冲区
        if (len1 <= len2 && len1 <= buffer_size)
        {
        ...
        {  // 区间长度太长，分割递归处理
            ...
            mySTL::merge_adaptive(first, first_cut, new_middle, len11,
                                len22, buffer, buffer_size, cmp);
            mySTL::merge_adaptive(new_middle, second_cut, last, len1 - len11,
                                len2 - len22, buffer, buffer_size, cmp);
        }
    }
    ...
    void
    inplace_merge(BidirectionalIter first, BidirectionalIter middle,
                BidirectionalIter last, Compared cmp)
    {
        if (first == middle || middle == last)
            return;
                ...
                Distance*, Compared cmp)
    {
        ...
        while (first != last)
        {
            ...
            {
                mySTL::adjust_heap(result_first, static_cast<Distance>(0),
                                result_iter - result_first, *first, cmp);
            }
            ++first;
        }
    ...
    {
        return mySTL::psort_copy_aux(first, last, result_first, result_last,
                                distance_type(result_first), cmp);
    }
    ...
    /* 
    partition_copy
    将被一元操作符判定为 true 的放到 result_true 的输出区间，其余放到 result_false 的输出区间
    返回一个 mySTL::pair 指向这两个区间的尾部
     */
    ...
    void intro_sort(RandomIter first, RandomIter last,
                    Size depth_limit, Compared cmp)
    {
        while (static_cast<size_t>(last - first) > kSmallSectionSize)
        {
            if (depth_limit == 0)
            {                            // 到达最大分割深度限制
                mySTL::partial_sort(first, last, last, cmp);  // 改用 heap_sort
                return;
            }
            --depth_limit;
            auto mid = mySTL::median(*(first), *(first + (last - first) / 2), *(last - 1));
    ...
    // 插入排序辅助函数 unchecked_linear_insert
    template <class RandomIter, class T, class Compared>
    void unchecked_linear_insert(RandomIter last, const T& value, Compared cmp)
    {
        auto next = last;
        --next;
   ...
   template <class RandomIter>
    ...
    OutputIter
    unique_copy_dispatch(InputIter first, InputIter last,
                        OutputIter result, output_iterator_tag, Compared cmp)
    {
        auto value = *first;
        *result = value;
        while (++first != last)
    ...
    template <class InputIter, class OutputIter, class Compared>
    OutputIter
    unique_copy(InputIter first, InputIter last, OutputIter result, Compared cmp)
    {
        if (first == last)
            return result;
        return mySTL::unique_copy_dispatch(first, last, result, iterator_category(result), cmp);
    }
